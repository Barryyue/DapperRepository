...
namespace CoreDapperRepository.Core.Infrastructure
{
    public class AppFileProvider : PhysicalFileProvider, IAppFileProvider
    {
        ...
        /// </summary>
        /// <param name="webRootPath">Hosting Web Root Path</param>
        public AppFileProvider(string webRootPath)
            : base(File.Exists(webRootPath) ? Path.GetDirectoryName(webRootPath) : webRootPath)
        {
            var path = webRootPath ?? string.Empty;
            if (File.Exists(path))
